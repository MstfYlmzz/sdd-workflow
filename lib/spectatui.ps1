...
function Get-SddSpectaStage {
    ...
    foreach ($name in @('converge','implement','analyze','tasks','plan','spec')) {
        $stage = Get-WorkflowProperty -Object $Ledger.stages -Name $name
        if ($stage -and [string](Get-WorkflowProperty -Object $stage -Name 'status' -Default '') -eq 'running') {
            return $name
        }
    }

    $implement = Get-WorkflowProperty -Object $Ledger.stages -Name 'implement'
    $converge = Get-WorkflowProperty -Object $Ledger.stages -Name 'converge'
    $convRound = [int](Get-WorkflowProperty -Object $converge -Name 'round' -Default 0)
    $implStatus = [string](Get-WorkflowProperty -Object $implement -Name 'status' -Default '')
    if ($convRound -gt 0 -and $implStatus -ne 'running') { return 'converge' }

    foreach ($name in @('implement','analyze','tasks','plan','spec')) {
        $stage = Get-WorkflowProperty -Object $Ledger.stages -Name $name
        $status = [string](Get-WorkflowProperty -Object $stage -Name 'status' -Default '')
...
function Write-SddSpectaStatus {
    ...
    $stageState = Get-WorkflowProperty -Object $Ledger.stages -Name $Stage
    if (-not $Status) {
        $Status = [string](Get-WorkflowProperty -Object $stageState -Name 'status' -Default 'not_started')
    }
    if (-not $StopReason) {
        $StopReason = [string](Get-WorkflowProperty -Object $stageState -Name 'stop_reason' -Default '')
    }

    if (-not $Profile -and $stageState) {
        $agent = [string](Get-WorkflowProperty -Object $stageState -Name 'agent' -Default '')
        $model = [string](Get-WorkflowProperty -Object $stageState -Name 'model' -Default '')
        $effort = [string](Get-WorkflowProperty -Object $stageState -Name 'effort' -Default '')
    ...
    if ($MaxAttempts -le 0 -and $config) {
        $MaxAttempts = [int](Get-WorkflowProperty -Object $config.loop -Name 'max_attempts' -Default 0)
    }
    $convergeState = Get-WorkflowProperty -Object $Ledger.stages -Name 'converge'
    if ($ConvergenceRound -lt 0) {
        $ConvergenceRound = [int](Get-WorkflowProperty -Object $convergeState -Name 'round' -Default 0)
    }
    if ($MaxConvergenceRounds -lt 0 -and $config) {
        $MaxConvergenceRounds = [int](Get-WorkflowProperty -Object $config.loop -Name 'max_converge_rounds' -Default 0)
    }
    ...
    $runtime = [ordered]@{
        ...
        agent = [string](Get-WorkflowProperty -Object $Profile -Name 'agent' -Default '')
        model = [string](Get-WorkflowProperty -Object $Profile -Name 'model' -Default '')
        effort = [string](Get-WorkflowProperty -Object $Profile -Name 'effort' -Default '')
    ...
    $doc = [ordered]@{
        schema_version = 1
        authoritative = $false
        spec_id = [string](Get-WorkflowProperty -Object $Ledger -Name 'spec_id' -Default '')
        stage = $Stage
        status = $Status
