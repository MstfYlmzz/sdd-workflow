...
impl CliAction {
    ...
    pub fn to_command_line(&self) -> String {
        match self {
            ...
            Self::SddStageRun { stage } => {
                format!("sdd {stage} -Ui raw")
            }
            Self::SddConfigSet {
                stage,
                agent,
                ...
                format!(
                    "sdd config set {stage} -Agent {agent} -Model {model} -Effort {effort}"
                )
            }
...
mod tests {
    ...
    fn sdd_control_actions_build_expected_commands() {
        ...
        assert_eq!(
            ...
            .to_command_line(),
            "sdd config set implement -Agent codex -Model gpt-5.6-sol -Effort high"
        );
    }
