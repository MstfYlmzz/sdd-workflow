...
fn draw_detail(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if wf.id == "sdd-native" {
        if let Some(sdd) = app.project.sdd_status.as_ref() {
            lines.push(Line::from(Span::styled(" SDD runtime", theme.accent_bold)));
            let status_style = match sdd.status.as_str() {
                "completed" => theme.good_style,
                "running" => theme.info_style,
                "failed" | "interrupted" | "blocked" => theme.warn_style,
                _ => theme.dim_style,
            };
            lines.push(Line::from(vec![
                Span::styled(" stage  ", theme.dim_style),
                Span::styled(sdd.stage.clone(), theme.accent_bold),
                Span::styled(" · ", theme.faint_style),
                Span::styled(sdd.status.clone(), status_style),
            ]));
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    format!(
                        ...
                        sdd.tasks.done, sdd.tasks.total, sdd.tasks.pending, sdd.tasks.blocked
                    ),
                    theme.info_style,
                ),
            ]));
            if !sdd.runtime.batch.is_empty() {
                lines.push(Line::from(vec![
            ...
            if !sdd.runtime.agent.is_empty() || !sdd.runtime.model.is_empty() {
                lines.push(Line::from(vec![
            ...
            if sdd.runtime.max_convergence_rounds > 0 {
                lines.push(Line::from(vec![
            ...
            if !sdd.runtime.stop_reason.is_empty() && sdd.runtime.stop_reason != "running" {
                lines.push(Line::from(vec![
                    Span::styled(" stop   ", theme.dim_style),
                    Span::styled(sdd.runtime.stop_reason.clone(), theme.warn_style),
                ]));
            }
            lines.push(Line::default());
        }
