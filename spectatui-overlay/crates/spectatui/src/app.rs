...
pub struct App {
    ...
    // SDD control center
    pub sdd_control_index: usize,
    pub sdd_route_editing: bool,
    pub sdd_route_field: usize,
    pub sdd_edit_agent: String,
    pub sdd_edit_model: String,
    pub sdd_edit_effort: String,
...
impl App {
    pub fn new(project: Project, config: AppConfig) -> Self {
        ...
        App {
            ...
            wf_index: 0,
            sdd_control_index: 0,
            sdd_route_editing: false,
            sdd_route_field: 0,
            sdd_edit_agent: String::new(),
            sdd_edit_model: String::new(),
            sdd_edit_effort: String::new(),
    ...
    pub fn open_popup(&mut self, kind: PopupKind) {
        match kind {
            ...
            PopupKind::SddControl => {
                self.sdd_control_index = self
                    .sdd_control_index
                    .min(self.project.sdd_config.as_ref().map(|c| c.routes.len().saturating_sub(1)).unwrap_or(0));
                self.sdd_route_editing = false;
                self.sdd_route_field = 0;
            }
            _ => {}
        }
        self.reset_filter();
    ...
    pub fn sdd_route_count(&self) -> usize {
        self.project
            .sdd_config
            .as_ref()
            .map(|cfg| cfg.routes.len())
            .unwrap_or(0)
    }

    pub fn sdd_select_next(&mut self) {
        let len = self.sdd_route_count();
        if len > 0 {
            self.sdd_control_index = (self.sdd_control_index + 1) % len;
        }
    }

    pub fn sdd_select_prev(&mut self) {
        let len = self.sdd_route_count();
        if len > 0 {
            self.sdd_control_index = if self.sdd_control_index == 0 {
                len - 1
            } else {
                self.sdd_control_index - 1
            };
        }
    }

    pub fn selected_sdd_route(&self) -> Option<&spectatui_core::speckit::SddRouteProfile> {
        self.project
            .sdd_config
            .as_ref()?
            .routes
            .get(self.sdd_control_index)
    }

    pub fn begin_sdd_route_edit(&mut self) {
        let Some(route) = self.selected_sdd_route().cloned() else {
            return;
        };
        self.sdd_route_editing = true;
        self.sdd_route_field = 0;
        self.sdd_edit_agent = route.agent;
        self.sdd_edit_model = route.model;
    ...
    pub fn cycle_sdd_agent(&mut self, delta: i32) {
        ...
        let current = AGENTS
            .iter()
            .position(|v| *v == self.sdd_edit_agent)
            .unwrap_or(0) as i32;
...
pub fn palette_commands() -> Vec<PaletteCommand> {
    vec![
        ...
        PaletteCommand {
            label: "SDD Control",
            hint: "D",
            action: PaletteAction::OpenPopup(PopupKind::SddControl),
        },
        PaletteCommand {
            label: "Help",
            hint: "?",
