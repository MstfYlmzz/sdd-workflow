...
function Get-CommonArguments {
    ...
    $r=[ordered]@{agent='';model='';effort='';ui='auto';select=$false;run_only=$false;remaining=[Collections.Generic.List[string]]::new()}; $a=@($Arguments)
    for($i=0;$i-lt$a.Count;$i++) { switch -Regex ($a[$i]) {
